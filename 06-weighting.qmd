...
::: {.fieldnote}
**In practice — a stratified facility survey.** Response was modeled for all 977 selected hospitals with a logistic regression on pre-survey attributes known for *every* hospital — stratum, facility size, and service-mix indicators carried on the sampling frame. Each responding hospital's nonresponse adjustment was 1 / its predicted response probability, so the hospitals least likely to respond carried the most weight. Overall response was 606 / 977 = 62%. Multiplying the two components gives the raw adjusted weight: w\_raw = w\_selection x w\_nonresponse.
:::
