...
Modules 0 through 9 taught you to **draw** a sample and **analyze** it honestly. This bonus module answers the question a study team asks **before** any of that: **how many people do we actually need?** That is a **power calculation**, and running it to get n is exactly what **determining a sample size** means. We anchor it to your **AAMC internship** task - comparing **two response rates** across invitation conditions. It is a cousin of Module 1's "sample size for a target precision," but here the target is a **difference to detect**, not a margin of error. 6 pieces of concept, then a hands-on seventh in R.
...
Sizing a study like this means setting the MDE at the smallest response-rate gap worth acting on, holding alpha at 0.05 and power at 80%, and solving for n per arm - then checking the reverse: what power the feasible n actually buys, so the team knows going in whether the study is powered to see the effect. Reporting both directions is what separates a sample size that was chosen from one that was assumed.
:::
...
Two base calls and the **pwr** package cover almost everything above. Here is the same calc in code.
